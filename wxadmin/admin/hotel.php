...
$exc = new exchange($ecs->table('hotels'), $db, 'link_id', 'link_name');
...
{
    /* 模板赋值 */
    $smarty->assign('ur_here',    '酒店列表');
    $smarty->assign('action_link', array('text' => '添加', 'href' => 'hotel.php?act=add'));
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('hotel_list.htm'), '',
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link', array('href'=>'hotel.php?act=list', 'text' => $_LANG['list_link']));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("link_name", $link_name) == 0)
    {
        ...
        /* 插入数据 */
        $sql    = "INSERT INTO ".$ecs->table('hotels')." (link_name, link_url, link_logo, show_order) ".
        ...
        $link[0]['href'] = 'hotel.php?act=add';

        $link[1]['text'] = $_LANG['back_list'];
        $link[1]['href'] = 'hotel.php?act=list';
...
{
    //admin_priv('hotel_list');

    /* 取得友情链接数据 */
    $sql = "SELECT link_id, link_name, link_url, link_logo, show_order ".
           "FROM " .$ecs->table('hotels'). " WHERE link_id = '".intval($_REQUEST['id'])."'";
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here',    '修改酒店');
    $smarty->assign('action_link', array('href'=>'hotel.php?act=list&' . list_link_postfix(), 'text' => '酒店列表'));
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    {
        //获取链子LOGO,并删除
        $old_logo = $db->getOne("SELECT link_logo FROM " .$ecs->table('hotels'). " WHERE link_id = '$id'");
    ...
    /* 更新信息 */
    $sql = "UPDATE " .$ecs->table('hotels'). " SET ".
            "link_name = '$link_name', ".
            "link_url = '$link_url' ".
            $link_logo.','.
    ...
    $db->query($sql);
    /* 记录管理员操作 */
    admin_log($_POST['link_name'], 'edit', 'hotel_list');
    ...
    $link[0]['href'] = 'hotel.php?act=list&' . list_link_postfix();
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'hotel.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
//-- 编辑地理坐标 x
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_location_x')
{
    check_authz_json('friendlink');
    ...
    {
        if ($exc->edit("hotel_location_x = '$order'", $id))
        {
            clear_cache_files();
            make_json_result(stripslashes($order));
        }
...
//-- 编辑地理坐标 y
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_location_y')
{
    check_authz_json('friendlink');
    ...
    {
        if ($exc->edit("hotel_location_y = '$order'", $id))
        {
            clear_cache_files();
            make_json_result(stripslashes($order));
        }
...
function get_links_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 获得总记录数据 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('hotels');
